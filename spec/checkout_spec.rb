...
describe('Checkout') do
  describe('#initialize') do
    it('will test the date methods') do
      new_checkout = Checkout.new({:book_id => 1, :patron_id => 1, :checkout_date => Date.new(2018, 06, 19), :id => nil})
  ...
  describe('#return') do
    it('will return a book and update the books in_stock status') do
      book1 = Book.new({:title => "Clifford the Big Red Dog", :author => 'John Smith', :id => nil})
      book1.save
      patron1 = Patron.new({:name => "Jane Smith", :id => nil})
      patron1.save
      ...
      expect(book1.in_stock?).to(eq(false))
      checkout1.return
      expect(book1.in_stock?).to(eq(true))
    end
  end
end
